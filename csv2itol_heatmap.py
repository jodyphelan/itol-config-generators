...
def main(args):
	...
	for row in csv.DictReader(open(args.csv)):
		meta_columns = set(row.keys())-set([args.id])
		for x in meta_columns:
			meta[row[args.id]][x] = row[x] if row[x] != "" else "NA"
		samples.append(row[args.id])

	columns = args.columns.split(",") if args.columns else sorted(meta_columns)
	...
	O = open("%s.heatmap.meta.itol.txt" % args.csv,"w")
